...
const fixString = (str) => {
	...
	let catType = str.slice(0, index + 1);
	let newStr = str.replace(catType, '');
	let noEek = newStr.replaceAll('Eek!', '');
	let reverseStr = noEek.split('').reverse().join('');
	let secretMessage = reverseStr
		.split('v')
		.map((substr) => substr[0])
		.join('')
		.slice(1);
